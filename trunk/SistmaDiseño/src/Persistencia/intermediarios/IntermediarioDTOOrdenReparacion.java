...
public class IntermediarioDTOOrdenReparacion extends IntermediarioRelacional {
    ...
    public String armarSelect(List<Criterio> criterios) {
        ...
        String select = "select * from ( "
                + "select denuncia.CodigoDenuncia, denuncia.OIDCaso, "
                ...
                + "interseccion.OIDUbicacion as OIDInterseccion, "
                + "interseccioncalle.OIDCalle as intercalle, "
                ...
                + "join caso on denuncia.OIDCaso = caso.OIDCaso "
                + "join casosemaforo on caso.OIDCaso = casosemaforo.OIDCaso "
                + "join semaforo on casosemaforo.OIDSemaforo = semaforo.OIDSemaforo "
                ...
                + "left join calle as calleUbicacionSimple on ubicacionsimple.OIDCalle = calleUbicacionSimple.OIDCalle "
                ...
                + "as denunciaCompleta "
                + "join ( "
                + "SELECT ordensemaforo.OIDOrdenDeTrabajo, ordensemaforo.FechaInicioTrabajo,ordensemaforo.FechaFinTrabajo, ordensemaforo.FechaInicioPlanificada, ordensemaforo.DuracionPrevistaTrabajo, ordensemaforo.Tipo, "
                + " ordenreparacion.CodigoOrdenReparacion,ordenreparacion.OIDDenuncia, "
                ...
                + "ordentrabajoestado.IndicadoresEstadoActual,"
                + "estadoordentrabajo.NombreEstado "
                + "from ordendetrabajo as ordensemaforo "
                + "join ordentrabajoestado on ordensemaforo.OIDOrdenDeTrabajo = ordentrabajoestado.OIDOrdenDeTrabajo "
                + "join estadoordentrabajo on ordentrabajoestado.OIDEstadoOrdenTrabajo = estadoordentrabajo.OIDEstadoOrdenTrabajo "
                + "join ordenreparacion on ordensemaforo.OIDOrdenDeTrabajo = ordenreparacion.OIDOrdenDeTrabajo "
                + "left join reserva on ordensemaforo.OIDOrdenDeTrabajo  = reserva.OIDOrdenDeTrabajo "
                ...
                + "join equipodetrabajo on ordensemaforo.OIDEquipoDeTrabajo = equipodetrabajo.OIDEquipoDeTrabajo) as ordenEn "
                + "on denunciaCompleta.OIDCaso = ordenEn.OIDDenuncia";

        String condicion = "";
        condicion = " WHERE ordenEn.FechaInicioPlanificada = '" + criterios.get(0).getValor() + "'";
        if (criterios.size() == 2) {
    ...
    public List<ObjetoPersistente> convertirRegistrosAObjetos(ResultSet rs) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                } else if (!estaOrdenTrabajo(listaOrdenes, rs.getString("CodigoOrdenReparacion")) && rs.getString("CodigoOrdenReparacion") != null) {
                    crearNuevaOrden = true;
                } else {
